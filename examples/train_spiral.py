...
from tasks.spiral import Spiral
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=[
            ...
            "max_cs=2500",
            "add_one_c_prob=0.5",
            "reenable_one_c_prob=0.15",
            "add_one_nd_prob=0.05",
            "enabled_recessiveness_prob=0.3",
...
if __name__ == "__main__":
    config = parse_args()
    train_task = Spiral(test=False, max_steps=config.max_rollout_steps)
    test_task = Spiral(test=True, max_steps=config.max_rollout_steps)
    ...
    img = Spiral.render(task_state, action)
